...
def get_schedule():
        ...
        for row in table.get('rows', []):
            ...
            for tid, tname in [(t1_id, t1_name), (t2_id, t2_name)]:
                                if tid and tid not in records:
                                                        records[tid] = {'w': 0, 'l': 0, 't': 0, 'pf': 0, 'pa': 0,
                                                                                                            'name': tname, 'weeklyPF': []}
            ...
            winner_name = t1_name if t1_won else (t2_name if t2_won is False else 'Tie')
    ...
    for tid, rec in records.items():
        ...
        # Compute streak from most recent periods (reverse order)
        weekly = rec['weeklyPF']
        streak_count = 0
        streak_type = None
        # Build per-period win/loss by comparing team PF vs opponent PF in each period
        # We'll compute streak in JS from the periods data instead
        rec['streak'] = ''  # Will be computed in JS from periods data
...
def get_transactions(tx_type):
        """Fetch recent transactions (waiver claims or MLB transactions)"""
